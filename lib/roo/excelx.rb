...
class Excelx < GenericSpreadsheet
  ...
  def date_base
    @workbook_doc.xpath("//*[local-name()='workbookPr']").attribute('date1904').value
  end
  ...
  def set_cell_values(sheet,x,y,i,v,vt,formula,tr,str_v,
    ...
    when :date
      ...
      @cell[sheet][key] = (date_base+v.to_i).strftime("%Y-%m-%d") 
    when :datetime
      @cell[sheet][key] = (datetime_base+v.to_f).strftime("%Y-%m-%d %H:%M:%S")
    when :percentage
      @cell[sheet][key] = v.to_f
    when :time
  ...
  def date_base
    epoch_attibute = @workbook_doc.xpath("//*[local-name()='workbookPr']").attribute('date1904')
    @date_base ||= if epoch_attibute && epoch_attibute.value == "1"
      Date.new(1904,1,1)
    else
      Date.new(1899,12,30)
    end
  ...
  def datetime_base
    @datetime_base ||= date_base.to_datetime
  end  
